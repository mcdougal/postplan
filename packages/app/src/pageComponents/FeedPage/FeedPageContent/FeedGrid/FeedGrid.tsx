...
const FeedGrid = ({
  ...
  const [isReloadFeedConfirmOpen, setIsReloadFeedConfirmOpen] = useState(false);
  ...
  return (
    <>
      <div className={twMerge(styles.phone, `relative px-2 pb-2 pt-0`)}>
        <div className="flex h-12 items-center justify-center gap-2">
          ...
          <IconButton
            ...
            label="Reload feed"
            onClick={() => {
              setIsReloadFeedConfirmOpen(true);
            }}
            size="sm"
          />
        </div>
      ...
      <ReloadFeedConfirmDialog
        onCancel={() => {
          setIsReloadFeedConfirmOpen(false);
        }}
        onConfirm={() => {
          if (onRefreshPosts) {
            onRefreshPosts();
        ...
        open={Boolean(onRefreshPosts && isReloadFeedConfirmOpen)}
      />
    </>
  );
};
